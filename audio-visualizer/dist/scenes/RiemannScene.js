...
export class RiemannScene extends BaseScene {
    constructor() {
        ...
        this.freqTexture = new THREE.DataTexture(this.freqData, 256, 1, THREE.RedFormat, THREE.UnsignedByteType);
    ...
    update(audio, time) {
        ...
        for (let i = 0; i < 256; i++) {
            const t = i / 256;
            this.freqData[i] = Math.floor((audio.bass * (1 - t) + audio.treble * t) * 255 * (0.5 + 0.5 * Math.sin(t * 20 + time * 3)));
        }
        this.freqTexture.needsUpdate = true;
    }
    dispose() {
