...
export class PostProcessing {
  ...
  private coralineBloomPass: ShaderPass;
  ...
  private readonly defaultBloom = { strength: 0.4, radius: 0.4, threshold: 1.0 };
  private isCoraline = false;
  ...
  constructor(renderer: THREE.WebGLRenderer, scene: THREE.Scene, camera: THREE.Camera) {
    ...
    this.bloom = new UnrealBloomPass(
      ...
      this.defaultBloom.strength,
      this.defaultBloom.radius,
      this.defaultBloom.threshold
    );
  ...
  updateScene(scene: THREE.Scene, camera: THREE.Camera) {
    ...
    const textPass = this.composer.passes[2] as RenderPass;
    textPass.scene = scene;
    textPass.camera = camera;
  }

  setBloomProfile(profile: 'default' | 'coraline') {
    if (profile === 'coraline') {
