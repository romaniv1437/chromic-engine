...
/**
 * Lyrics word data from timeline.
 */
...
function wordType(w: LyricsWord): string {
  if (w.stretch || w.isVocalStretch) return 'stretch';
...
function configureTextMesh(mesh: any, text: string, fontSize: number) {
  mesh.text = text;
  mesh.font = FONT;
...
function lineState(li: number, ai: number, isAdlib: boolean, isScrolling: boolean, isPaired?: boolean) {
  // During manual scroll: all lines equally visible (matches DOM .is-scrolling-active)
  if (isScrolling) {
    return li === ai ? S.scrollAct : S.scroll;
  }
  if (isAdlib) return li === ai ? S.adlibOn : li < ai ? S.adlibOff : S.adlibHid;
  // Paired () echo lines: dedicated states from gpu-panel
  if (isPaired) {
    if (li === ai) return S.pairAct;
    return li < ai ? S.pairPast : S.pairFut;
  }
  if (li === ai) return S.active;
  if (li < ai) return (ai - li > 1) ? S.pastFar : S.past;
  const d = li - ai;
...
interface WordEntry {
  ...
  _isCueDot?: boolean;
  _dotRadius?: number;
  // ── Char-split (sung/stretch per-character wave) ──
  _isCharSplit?: boolean;
  _chars?: CharMesh[];
  _charWidths?: number[];
  _charCount?: number;
  // ── Adlib phrase grouping ──
  _adPhraseIdx?: number;
  _adBaseY?: number;
  _adBaseX?: number;
  _baseX?: number;
...
interface RenderedLine {
  group: THREE.Group;
  entries: WordEntry[];
  ld: { start: number; end: number };
  ad: boolean;
  isCue: boolean;
  hasInlineAdlibs: boolean;
  fs: number;
  _baseY: number;
  _cOp: number;
  _cSc: number;
  ...
  // ── Paired () echo lines ──
  _pairWith?: number;
  _pairRole?: 'first' | 'second';
  // ── Standalone adlib lines (full adlib, not paired — shown like normal lines) ──
  _standaloneAdlib?: boolean;
  // ── Inline adlib phrases ──
  _adPhrases?: AdlibPhrase[];
  _adLastEnd?: number;
  _adPhraseReveal?: number;
  // ── Cue dot collapse animation ──
  _cueCollapse?: number;      // 0 = full size, 1 = fully collapsed (no space)
  _cueCollapseTarget?: number;
  _cueExitPhase?: number;     // animation phase: 0=active, 1=bloom-up, 2=shrink-away
...
export class LyricsRenderer {
  ...
  private ready = false;
  private lastCt = 0;        // for seek detection
  private _pendingSeekTime: number | null = null;  // set by handleClick, consumed by update
  private _leftEdge = 0;     // layout left edge
  private _aspectX = 1;      // X scale correction for aspect ratio (h/w)
  // ── Scroll state (from gpu-panel) ──
  private manualScrollOffset = 0;
  private scrollDecayTimer = 0;
  private scrollY = ACTIVE_LINE_Y_CONTROLS;  // actual interpolated scroll position (like gpu-panel cameraY)
  // ── Seek callback ──
  private _onSeek: ((time: number) => void) | null = null;
  // ── Track metadata display (artist + album) ──
  private _metaGroup: THREE.Group | null = null;
  private _metaArtistMesh: any = null;
  private _metaAlbumMesh: any = null;
  private _metaVisible = false;
  // ── Debug: pixel color picker ──
  private _debugRenderer: THREE.WebGLRenderer | null = null;
  private _debugEnabled = false;
  private _debugLastDump = 0;
  // ── UI visibility — shifts text position when controls shown ──
  private _uiVisible = true;
  private _activeLineY = ACTIVE_LINE_Y_CONTROLS;
  private _accentColor: THREE.Color = new THREE.Color(0x7eb8ff); // default blue
  ...
  setTimeline(timeline: LyricsLine[], artist?: string, album?: string) {
    // Dispose old
    this.disposeLines();
  ...
  private disposeLines() {
    // Dispose track metadata
    if (this._metaGroup) {
      if (this._metaArtistMesh) { this._metaArtistMesh.dispose(); this._metaArtistMesh = null; }
      if (this._metaAlbumMesh) { this._metaAlbumMesh.dispose(); this._metaAlbumMesh = null; }
    ...
    for (const L of this.lines) {
      for (const e of L.entries) {
        if (e._isCueDot) {
          (e.base as THREE.Mesh).geometry?.dispose();
          ((e.base as THREE.Mesh).material as THREE.Material)?.dispose();
          (e.fill as THREE.Mesh).geometry?.dispose();
          ((e.fill as THREE.Mesh).material as THREE.Material)?.dispose();
  ...
  private buildScene(raw: LyricsLine[]) {
    ...
    for (let i = 0; i < withCues.length; i++) {
      ...
      if (!sl.words || !sl.words.length) {
        ...
        sl.words = tokens.map((word: string, wi: number) => ({
    ...
    for (let li = 0; li < src.length; li++) {
      ...
      const isAllAdlib = !isCue && ((sl.words || []).every((w: any) => w.adlib) || !!sl.adlib);
      const hasInlineAdlibs = !isCue && !isAllAdlib && (sl.words || []).some((w: any) => w.adlib);
      ...
      if (isCue) {
        ...
        for (let wi = 0; wi < 3; wi++) {
          ...
          // Enable both layers: 0 for main render, 1 for bloom pass
          base.layers.enable(0);
          ...
          entries.push({
            ...
            ad: false, wt: 'normal',
            _w: dotRadius * 2, _p: 0, _cop: 1, _tOp: 1,
      ...
      for (let wi = 0; wi < sl.words.length; wi++) {
        ...
        if (isSungStretch && t.length > 1 && !isAllAdlib) {
          ...
          for (let ci = 0; ci < chars.length; ci++) {
            const ch = chars[ci];
            const cBase: any = new Text();
            configureTextMesh(cBase, ch, wordFs);
            cBase.layers.set(1);
            g.add(cBase);

            const cFill: any = new Text();
      ...
      const adPhrases: AdlibPhrase[] = [];
      if (hasInlineAdlibs) {
        let curPhrase: AdlibPhrase | null = null;
        for (const e of entries) {
          if (e.ad) {
            ...
            if (curPhrase) { curPhrase.totalW -= WORD_GAP; adPhrases.push(curPhrase); curPhrase = null; }
          }
        }
        if (curPhrase) { curPhrase.totalW -= WORD_GAP; adPhrases.push(curPhrase); }
    ...
    for (const L of this.lines) {
      for (const e of L.entries) {
        if (e._isCueDot) {
          syncDone += 2;
          if (syncDone >= syncCount) { this.doLayout(); this.ready = true; this.group.visible = true; this.targetOpacity = 1; }
        } else if (e._isCharSplit) {
          for (const cm of e._chars!) {
            cm.base.sync(onSync);
            cm.fill.sync(onSync);
  ...
  private layoutChars(e: WordEntry, wordX: number, wordY: number) {
      ...
      if (L.hasInlineAdlibs) {
        ...
        for (const e of mainWords) {
          if (x + e._w > leftEdge + maxWidth && x > leftEdge + 0.01) {
            x = leftEdge; rowY -= rowHeight;
          }
          e.base.position.x = x; e.base.position.y = rowY;
          e.fill.position.x = x; e.fill.position.y = rowY;
          e._adBaseY = rowY; e._baseY = rowY;
          e._baseX = x;
          if (e._isCharSplit) this.layoutChars(e, x, rowY);
          x += e._w + WORD_GAP;
        }
        ...
        for (const e of adlibWords) {
          e.base.position.x = adX; e.base.position.y = adRowY;
          e.fill.position.x = adX; e.fill.position.y = adRowY;
          e._adBaseY = adRowY; e._baseY = adRowY;
          e._baseX = adX;
          e._adBaseX = adX;
          if (e._isCharSplit) this.layoutChars(e, adX, adRowY);
          adX += e._w + WORD_GAP;
        }
  ...
  setTrackMeta(artist?: string, album?: string) {
    ...
    if (!artistText && !albumText) { this._metaVisible = false; return; }
  ...
  private updateMeta() {
    ...
    const bottomY = lastLine ? lastLine._baseY - LINE_HEIGHT * 5 : -0.5;
    ...
    let fadeMeta = 1.0;
    if (screenY > 1.2) fadeMeta = 0;
    else if (screenY < -1.8) fadeMeta = 0;
    else if (screenY > 0.9) fadeMeta = Math.max(0, (1.2 - screenY) / 0.3);
    else if (screenY < -1.3) fadeMeta = Math.max(0, (screenY + 1.8) / 0.5);
    ...
    if (this._metaArtistMesh?.material) {
      ...
      // Colored bloom using palette accent
      if (artistOp > 0.1) {
        this._metaArtistMesh.layers.enable(0);
        const boost = 1.0 + 0.25 * fadeMeta;
        const c = this._accentColor;
        this._metaArtistMesh.color = new THREE.Color(
          c.r * boost,
          c.g * boost,
          c.b * boost
        );
  ...
  handleClick(ndcX: number, ndcY: number) {
    if (!this.ready || !this.lines.length) return;
    if (ndcX < TEXT_LEFT - 0.2) return; // click is on left side (shader area), ignore

    // Convert NDC click to layout space (group.scale.x = _aspectX)
    const clickX = ndcX / (this._aspectX || 1);
    let bestEntry: WordEntry | null = null;
    ...
    for (const L of this.lines) {
      ...
      for (const e of L.entries) {
        ...
        if (dy > L.fs * 2.5) continue; // wider Y threshold
        const wordLeft = wx;
        ...
        if (dist < bestDist) { bestDist = dist; bestEntry = e; bestLine = L; }
      }
    }
    ...
    for (let i = 0; i < this.lines.length; i++) {
      if (this.lines[i]._cOp < 0.05) continue;
    ...
    if (this._onSeek && this.lines[closest]) {
      ...
      for (const e of this.lines[closest].entries) e._clickFlash = 1.0;
    }
  }
  ...
  update(rms: number) {
    ...
    if (this._dbgCount % 300 === 1 && false) { // DEBUG: set to true to enable verbose frame logging
      ...
      for (let li = from; li <= to; li++) {
        ...
        const lineInfo = `[LR-LINE] #${li} ${rel} | cOp=${L._cOp.toFixed(3)} cSc=${L._cSc.toFixed(3)} baseY=${L._baseY.toFixed(3)} grpY=${L.group.position.y.toFixed(3)} isCue=${L.isCue} ad=${L.ad} paired=${!!(L as any).pairedAdlibIdx}`;
        console.log(lineInfo);
        // Dump each word in the line
        for (let wi = 0; wi < L.entries.length; wi++) {
          const e = L.entries[wi];
          if (e._isCueDot) { console.log(`  w${wi}: [cue-dot]`); continue; }
          ...
          const clipStr = e.fill?.clipRect ? `[${e.fill.clipRect.map((v: number) => v.toFixed(2)).join(',')}]` : 'null';
          const charInfo = e._isCharSplit ? ` chars=${e._charCount} charW=[${(e._charWidths||[]).map((w: number) => w.toFixed(3)).join(',')}]` : '';
    ...
    if (didSeek) {
      ...
      for (let i = 0; i < this.lines.length; i++) {
        ...
        if (ct >= ld.start && ct <= ld.end) { seekAi = i; break; }
        if (ct > ld.end) seekAi = i;
      }
        ...
        for (const e of L.entries) {
          ...
          if (e._isCueDot) {
            ...
            for (const cm of e._chars) {
              ...
              (cm as any)._lerpScX = 1;
              (cm as any)._lerpScY = 1;
              (cm as any)._charGlow = 0;
              if ((cm as any)._baseX !== undefined) {
                cm.base.position.x = (cm as any)._baseX;
                cm.fill.position.x = (cm as any)._baseX;
              }
            }
          } else {
    ...
    for (let i = 0; i < this.lines.length; i++) {
      const ld = this.lines[i].ld;
      if (ct >= ld.start && ct <= ld.end) { ai = i; break; }
    }
    if (ai === -1) {
      for (let i = 0; i < this.lines.length; i++) {
        const ld = this.lines[i].ld;
        if (ct >= ld.start - 0.15 && ct <= ld.end + 0.15) { ai = i; break; }
      }
    }
    ...
    if (ai === -1) ai = this.activeLineIdx >= 0 ? this.activeLineIdx : 0;
    ...
    if (ai >= 0 && ai < this.lines.length) {
      ...
      if (pairIdx != null && pairIdx >= 0 && pairIdx < this.lines.length) {
        ...
        if (ct <= pairEnd + 0.1) ai = Math.min(ai, pairIdx);
      }
      for (let i = 0; i < this.lines.length; i++) {
        ...
        if (overlapDur > 0.05) activeSet.add(i);
      }
      if (activeSet.size > 1) {
        let maxEnd = 0;
        for (const idx of activeSet) maxEnd = Math.max(maxEnd, this.lines[idx].ld.end);
        if (ct <= maxEnd + 0.1) {
    ...
    if (this.scrollDecayTimer > 0) {
      ...
      if (Math.abs(this.manualScrollOffset) < 0.005) this.manualScrollOffset = 0;
    }
    ...
    for (let li = 0; li < this.lines.length; li++) {
      ...
      L._tOp = st.op; L._tSc = st.sc; L._tOy = st.oy;
      ...
      if (isPast) {
        ...
        const delta = li - ai;
        let staggerF: number;
        if (delta === 1) { staggerF = f * 0.8; }       // future-1: fairly responsive
        else if (delta === 2) { staggerF = f * 0.5; }       // future-2: moderate lag
        else { staggerF = f * Math.max(0.2, 0.4 - delta * 0.04); } // distant = slow cascade
      ...
      if (screenY > 1.0 - MARGIN) viewportFade = Math.max(0, (1.0 - screenY) / MARGIN);
      else if (screenY < -1.0 + MARGIN) viewportFade = Math.max(0, (screenY + 1.0) / MARGIN);
      ...
      for (const e of L.entries) {
        ...
        if (L.isCue && e._isCueDot) {
          const baseMesh = e.base as THREE.Mesh;
          const fillMesh = e.fill as THREE.Mesh;
          const baseMat = baseMesh.material as THREE.MeshBasicMaterial;
          ...
          let dotFill: number;
          if (ct >= effectiveEnd) dotFill = 1;
          else if (ct >= e.start) dotFill = Math.min(1, (ct - e.start) / Math.max(0.01, effectiveEnd - e.start));
          else dotFill = 0;
        ...
        if (e._isCharSplit && e._chars) {
          ...
          for (let ci = 0; ci < n; ci++) {
            ...
            const prevScY = (cm as any)._lerpScY ?? 1;
            const prevScX = (cm as any)._lerpScX ?? 1;
            const lf = 1 - Math.exp(-3.5 * dt);
            const scY = prevScY + (targetScY - prevScY) * lf;
            const scX = prevScX + (targetScX - prevScX) * lf;
            (cm as any)._lerpScY = scY;
            (cm as any)._lerpScX = scX;
            ...
            // Track per-char glow: wave sets it, then it decays slowly = trailing halo
            if (wave > (cm as any)._charGlow) (cm as any)._charGlow = wave;
            else (cm as any)._charGlow = ((cm as any)._charGlow || 0) * 0.97; // slow decay = long trail

            // Track peak wave for whole-word glow
            if (wave > e._glow) e._glow = wave;
            else e._glow *= 0.92;

            // BLOOM: HDR boost on any char that has residual glow (filled chars keep halo trail)
            const charGlow = (cm as any)._charGlow || 0;
            if (charGlow > 0.02 && isActive) {
              const boost = 1.0 + 0.15 * charGlow;
        ...
        if (e.base.material) e.base.material.opacity = baseOpacity;
        if (e.fill.material) e.fill.material.opacity = fillOpacity;
        ...
        if (isPast || isFilled) {
          ...
          if (_lineP >= 1) {
            ...
            if (localClip <= 0) e.fill.clipRect = [-10, -10, -10, 10];
            else if (localClip >= e._w) e.fill.clipRect = [-10, -10, 100, 10];
            else e.fill.clipRect = [-0.01, -10, localClip, 10];
          }
        } else if (e._p <= 0) {
          e.fill.clipRect = [-10, -10, -10, 10];
        } else {
        ...
        if (isStretch) {
          ...
          if (isActive2) {
            ...
            // Oscillation: continuous sinusoidal shear
            const freq = Math.max(2.5, 6.0 / Math.max(0.3, wordDur));
        ...
        if (absWave > 0.01) {
          // Gentle scale pulse synced to wave amplitude
          const scalePulse = absWave * 0.06;
          tScY = 1 + scalePulse;
          tScX = 1 + scalePulse * 0.4;
          e._glow += (absWave - e._glow) * 0.18;
        } else {
          e._glow *= 0.97; // slow decay = long trailing halo
          if (e._glow < 0.005) e._glow = 0;
        }
        // Love wave horizontal displacement (transverse to propagation)
        const loveOffsetX = e._wave * FONT_SIZE * 0.35;

        // Layer-based bloom: HDR color boost during wave (equivalent to gpu-panel's outlineWidth glow)
        if (e._glow > 0.005 && isActive) {
          const g = e._glow;
          const g2 = g * g;
          // Bright core intensity boost — subtle, up to 1.15x
          const boost = 1.0 + 0.15 * g2;
          e.fill.color = new THREE.Color(boost, boost, boost);
          e.fill.material.opacity = Math.min(1.0, (e.fill.material?.opacity || fillOpacity) * (1.0 + 0.15 * g2));
          // Enable layer 0 for bloom pass
          e.fill.layers.enable(0);
        } else {
          e.fill.color = 0xffffff;
          // Remove from bloom pass
          e.fill.layers.disable(0);
        }
        ...
        if (!e.ad || !L.hasInlineAdlibs) {
          ...
          if (e._baseX === undefined) e._baseX = e.base.position.x;
        ...
        if (e.ad && L.hasInlineAdlibs && L._adPhrases) {
          ...
          const lastEnd = L._adLastEnd || 0;
          const timeUntil = phrase.start - ct;
          let phraseVisible: boolean;
          if (!isActive && ct > lastEnd) phraseVisible = false;
          else if (ct >= phrase.start - 0.3) phraseVisible = true;
          else phraseVisible = false;
          ...
          if (isPast || isFilled) e.fill.clipRect = [-10, -10, 100, 10];
          else if (e._p <= 0) e.fill.clipRect = [-10, -10, -10, 10];
          else {
            const clipX = e._w * e._p;
            const feather = e._w * FEATHER;
            e.fill.clipRect = [-0.01, -10, clipX + feather, 10];
   ...
   */
  debugReadPixel(renderer: THREE.WebGLRenderer, canvasX: number, canvasY: number): { r: number; g: number; b: number; a: number } | null {
    try {
   ...
   */
  debugInspectClick(ndcX: number, ndcY: number, renderer: THREE.WebGLRenderer, clientX: number, clientY: number) {
    ...
    if (pixel) {
      for (let dy = -2; dy <= 2; dy++) {
        for (let dx = -2; dx <= 2; dx++) {
          ...
          if (p && p.a > 0) samples.push(`(${p.r},${p.g},${p.b},${p.a})`);
        }
      }
    }
    ...
    for (let li = 0; li < this.lines.length; li++) {
      ...
      for (const e of L.entries) {
        ...
        const wx = L.group.position.x + e.base.position.x * groupScale;
        const dy = Math.abs(ndcY - wy);
        if (dy > L.fs * 2) continue;
        const wordRight = wx + e._w * groupScale;
        const dist = dy + Math.abs(clickX - (wx + wordRight) / 2) * 0.3;
        if (dist < bestDist) { bestDist = dist; bestEntry = e; bestLine = L; bestLi = li; }
      }
    }
    ...
    if (samples.length) console.log(`  5×5 non-transparent samples (${samples.length}): ${samples.slice(0, 8).join(' ')}`);
    ...
    if (bestEntry && bestLine) {
      ...
      if (e._isCharSplit && e._chars) {
        ...
        for (const cm of e._chars) {
          ...
          const clipStr = cm.fill?.clipRect ? `[${cm.fill.clipRect.map((v: number) => v.toFixed(2)).join(',')}]` : 'null';
      ...
      if (pixel && pixel.a > 0) {
        ...
        console.log(`  %cExpected brightness ~${expectedBrightness} | Actual pixel brightness ~${actualBrightness} | Delta: ${actualBrightness - expectedBrightness}`,
          Math.abs(actualBrightness - expectedBrightness) > 20 ? 'color: #f00; font-weight: bold' : 'color: #0f0');
      }
    } else {
   ...
   */
  private debugPeriodicDump(ct: number, ai: number) {
    if (!this._debugEnabled) return;
    if (ct - this._debugLastDump < 5) return; // every 5 sec
