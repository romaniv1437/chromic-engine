...
export class LorenzScene extends BaseScene {
  private gpuCompute: { rt1: THREE.WebGLRenderTarget; rt2: THREE.WebGLRenderTarget; simMat: THREE.ShaderMaterial; mesh: THREE.Mesh };
  ...
  constructor(renderer: THREE.WebGLRenderer) {
    ...
    // Render targets
    const rtOpts: THREE.RenderTargetOptions = { minFilter: THREE.NearestFilter, magFilter: THREE.NearestFilter, format: THREE.RGBAFormat, type: THREE.FloatType };
  ...
  update(audio: AudioData, time: number) {
    ...
    (this.camera as THREE.PerspectiveCamera).lookAt(0, 0, 0);
  }
